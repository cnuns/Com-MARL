...
from com_marl.torch.modules import CategoricalLSTMModule, MLPEncoderModule

class DecCategoricalLSTMPolicy(nn.Module):
    def __init__(self,
                 env_spec,
                 n_agents,
        ...
        self.centralized = True # ccentralized training! Using centralized sampler
        self.vectorized = True
        
        self._n_agents = n_agents
        self._obs_dim = int(env_spec.observation_space.flat_dim / n_agents) # dec obs_dim
        self._action_dim = env_spec.action_space.n
        self._embedding_dim = self._obs_dim # embedding_dim
    ...
    def step_forward(self, obs_n, avail_actions_n, get_actions=True):
        """
        ...
        inputs = inputs.reshape(1, n_envs * self._n_agents, -1)
        ...
        masked_probs = dists_n.probs.squeeze(0).reshape(n_envs, self._n_agents, self._action_dim)
        masked_probs = masked_probs.cpu() * torch.Tensor(avail_actions_n) # mask
    ...
    def get_actions(self, obs_n, avail_actions_n, greedy=False):
        ...
        """
        with torch.no_grad():
            dists_n = self.step_forward(obs_n, avail_actions_n, get_actions=True)
            if not greedy:
