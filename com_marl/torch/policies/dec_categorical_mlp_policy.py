...
from com_marl.torch.modules import CategoricalMLPModule
...
class DecCategoricalMLPPolicy(Policy, CategoricalMLPModule):
    ...
    def __init__(self,
        ...
        encoder_hidden_sizes = (hidden_sizes[0], ) # (128, )
        self._embedding_dim = hidden_sizes[1] # 64
        hidden_sizes = (hidden_sizes[-1], ) # 32
        ...
        self.encoder = MLPEncoderModule(input_dim=self._obs_dim,
    ...
    def forward(self, obs, avail_actions, get_actions=False):
        if get_actions:
        ...
        avail_actions = avail_actions.reshape(avail_actions.shape[:-1] + (self._n_agents, -1))
        if get_actions:
            masked_probs = dist.probs.cpu() * torch.Tensor(avail_actions) # mask 
        else:
    ...
    def get_actions(self, observations, avail_actions, greedy=False):
        ...
        with torch.no_grad():
            ...
            dist = self.forward(torch.Tensor(observations), torch.Tensor(avail_actions), get_actions=True)
