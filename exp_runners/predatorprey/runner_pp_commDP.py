...
from com_marl.torch.policies import CommCategoricalMLPPolicy # policy
...
def run(args):
    ...
    if args.mode == 'train':
        ...
        def train_predatorprey(ctxt=None, args_dict=vars(args)):
            ...
            policy, baseline = commDP(args, env)
...
if __name__ == '__main__':
    ...
    if 'train' in args.cmd:
        args = ready_to_train(args, pp, flag, debug=0) # pass here if model already exist OR start to train new model
        run(args)
