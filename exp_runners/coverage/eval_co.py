...
def eval_model(env, policy, itr, n_eval_episodes=100, max_env_steps=200,
    ...
    for i_eps in range(n_eval_episodes):
        ...
        for i_step in range(max_env_steps):
            ...
            if agent_dones or i_step == max_env_steps - 1:
                ...
                epi_optRew.append(env.bound_return)
                for vec in VECTORS:
                    if vec == 'nodeDeg':
    ...
    print(f'### Average Reward = {(sum(eval_rewards) / len(eval_rewards)) / (sum(epi_optRew)/len(epi_optRew))}')
    ...
    return episode_data, epi_success, epi_rewards, epi_optRew
